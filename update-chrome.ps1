# Force update Google Chrome to the latest version
# Bypasses local and app-specific policies, stops existing Chrome processes, and invokes Google Update.

Write-Host "Checking for administrator update blocks..."
$policyPath = "HKLM:\SOFTWARE\Policies\Google\Update"

# Temporarily allow updates if the policy key exists
if (Test-Path $policyPath) {
    Write-Host "Overriding Google Update policies for this session to allow manual updates..."
...
foreach ($updateExe in $googleUpdatePaths) {
    ...
    if ($LASTEXITCODE -ne 0) {
        Write-Warning "Updater returned exit code $LASTEXITCODE for $updateExe"
    }
}
