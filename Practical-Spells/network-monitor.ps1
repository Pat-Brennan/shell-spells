...
#LOW SPEED NETWORK MONITOR DASHBOARD

# $computers = 1..255 | ForEach-Object { "10.5.1.$_" }

# # This loop runs until you press Ctrl+C
# while ($true) {
#     Clear-Host
#     Write-Host "--- Network Live Monitor (Press Ctrl+C to Stop) ---" -ForegroundColor Yellow
#     Write-Host "Last Updated: $(Get-Date -Format 'HH:mm:ss')" -ForegroundColor Gray
#     Write-Host ("-" * 50)

#     $results = $computers | ForEach-Object -Parallel {
#         $ping = Test-Connection -ComputerName $_ -Count 1 -Quiet -TimeoutSeconds 1
        
#         if ($ping) {
#             [PSCustomObject]@{
#               IP = $_; Status = "UP"; Color = "Green";
#             } 
#         } else {
#               [PSCustomObject]@{
#                 IP = $_; Status = "DOWN"; Color = "Red";
#               }
#         }
#       } -ThrottleLimit 255

#       $upCount = ($results | Where-Object Status -eq "UP").Count
#       Write-Host "Summary: $upCount Hosts Up | $($results.Count - $upCount) Hosts Down" -ForegroundColor Cyan
#       Write-Host ("-" * 50)

#       foreach ($item in ($results | Sort-Object {[version]$_.IP})) {
#         Write-Host "[$($item.Status)]" -ForegroundColor $item.Color -NoNewline
#         Write-Host " $($item.IP)"
#     }
#     Start-Sleep 10 
#   }
